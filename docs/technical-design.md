...
- 楽天へ登録した公開ホスト名を許可サイトとして使用する
- 公開URLを取得するまでは同じAPI契約のモック実装で開発する
...
画面内の「情報について」に、楽天Books APIを利用していること、情報の完全性を保証しないこと、訂正窓口を設けていないことを記載する。楽天Web Serviceの利用条件に従い、必要なクレジットと楽天の商品ページへの出典リンクを表示する。表示文言、リンク形式、書影利用方法は、実装時点の利用規約を再確認して確定する。
